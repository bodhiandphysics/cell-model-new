...
Sim::Sim(glm::vec2 startpos, float alength, int numcellsx, int numcellsy, float wall_alpha, float ang_alpha) { // I should fix this so it doesn't just do a grid
	...
	for (int i = 0; i < numcellsy; i++)
		for (int j = 0; j < numcellsx; j++) {
			...
			cells.emplace_back(cell_pos,  float(i+1) * glm::mat2{grate}, 4, M_PI/2, wall_alpha, ang_alpha);
			std::cout << "made a wall";
	}

}

void Sim::sim_sub_iteration(float dt, float& convergence_distance_last) {
...
void Sim::sim_iteration(float dt) {
	...
	sim_sub_iteration(dt, convergence_distance_last);
	...
	std::cout<<"Current error: "<< convergence_distance_last <<std::endl;
